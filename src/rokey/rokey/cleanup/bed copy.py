...
class RobotController(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(f"[DEBUG] Service name: {self.get_position_client.srv_name}")
...
#커피와 동일
    def get_target_pos(self, target):
        self.get_logger().info(f"[요청] Target: {target}")
        ...
        self.get_logger().info(f"{get_position_future}")
        ...
        while rclpy.ok():
            ...
            if blanket_box and not bed_box and not pillow_box:
                ...
                if bx1 <= px <= bx2 and by1 <= py <= by2:
                    ...
                    self.get_logger().info("배개도 침대 아래")
...
#이불 각도 및 중심 계산
    ...
    def robot_control(self, msg):
        self.clean_move_triger = msg.data
        if self.clean_move_triger == False:
            movej([0,0,90,0,90,0], vel=VELOCITY, acc=ACC)
            while rclpy.ok():
