...
class VoiceServiceNode(Node):
    def __init__(self):
        ...
        <주제 리스트>
        - meal, weather, cleanup, exception

        <선정 규칙>
        - 커피나 식사, 시리얼에 관한 명령이면 meal을 출력해줘
        - 날씨에 관한 질문이나 노래 추천, 의상 추천에 관한 이야기이면 weather을 출력해줘
        - 나 나갈게나 집 잘지켜줘 등에 외출 인사를 하면 cleanup를 출력해줘
        - 위에 규칙과 관련이 없는 대화면 주제리스트에서 exception을 출력해줘
        ...
        self.mic_config = MicConfig(
            ...
            fmt=pyaudio.paInt16, device_index=8, buffer_size=24000
        )
    ...
    def run_loop(self):
        while rclpy.ok():
            ...
            if subject == "exception":
                result = self.generate_response(subject, text_input)
                self.get_logger().warn(f"⚠️ 예외 응답: {result}")
                continue
    ...
    def build_chain_for_subject(self, subject, user_input):
        if subject == "meal":
            ...
            - 들어온 텍스트의 오타가 있을 경우 가장 비슷한 맛으로 고쳐주세요 ex) 쎈만 -> 쓴만 / 콤프로스트 -> 콘프로스트
